...
try:
    ...
    from proof_agent.paths import MARKDOWN_CACHE_DIR, resolve_from_project
except Exception:
    ...
    MARKDOWN_CACHE_DIR = Path(".cache/markitdown")

    def resolve_from_project(raw_path, *, empty_ok=False):
        text = str(raw_path or "").strip()
        if not text:
            return "" if empty_ok else os.getcwd()
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "inputs",
        nargs="+",
        help="输入的 PDF 文件、目录，或 glob 模式，如 data/papers/references/*.pdf",
    )
    parser.add_argument(
        "-o",
        "--output-dir",
        default=str(MARKDOWN_CACHE_DIR),
        help=f"Markdown 输出目录，默认是 {MARKDOWN_CACHE_DIR}",
    )
    parser.add_argument(
        "--recursive",
        action="store_true",
...
def main() -> int:
    ...
    output_dir = Path(resolve_from_project(args.output_dir))
