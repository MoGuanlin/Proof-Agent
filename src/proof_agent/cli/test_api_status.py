...
def main():
    ...
    parser.add_argument("--provider", default="google", help="google or siliconflow; default from env LLM_PROVIDER")
    ...
    if not model:
        model = "gemini-3.1-pro-preview" if provider == "google" else "Pro/deepseek-ai/DeepSeek-V3.2"

    api_key = (
        (os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY") or "").strip()
        if provider == "google"
        else (os.getenv("SILICONFLOW_API_KEY") or "").strip()
    )
    if not api_key:
