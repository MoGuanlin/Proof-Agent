...
class CandidateExplorationPipeline:
    ...
    @staticmethod
    def _property_passed(candidate, property_name):
        detail = (candidate.property_status or {}).get(str(property_name).strip()) or {}
        return str(detail.get("status", "")).strip().lower() == "pass"

    def _pending_properties_for_candidate(self, candidate):
        ordered = []
        seen = set()
        for prop in (candidate.priority or self.system.property_order):
            if prop not in self.system.property_order or prop in seen:
                continue
            seen.add(prop)
            if not self._property_passed(candidate, prop):
                ordered.append(prop)
    ...
    def _restore_latest_candidates(self):
        latest_candidates = list(self.system.memory.load_latest_candidates() or [])
        active_candidates = [candidate for candidate in latest_candidates if candidate.status == "active"]
        active_candidate = active_candidates[-1] if active_candidates else None
        return latest_candidates, active_candidate
    ...
    def run(self):
        latest_candidates, active_candidate = self._restore_latest_candidates()
        next_candidate_index = 1

        if active_candidate is not None:
            self.passed_candidates = [candidate for candidate in latest_candidates if candidate.status == "passed"]
            self.pruned_candidates = [candidate for candidate in latest_candidates if candidate.status == "pruned"]
            next_candidate_index = max(1, len(latest_candidates))
            print(f"🔄 恢复 active 候选: {active_candidate.candidate_id}")
            active_candidate.append_log(
                "resume",
                "resumed active candidate from latest snapshot",
                next_property=(self._pending_properties_for_candidate(active_candidate) or ["[none]"])[0],
            )
            self.system.memory.save_candidate(active_candidate)
            self._explore_candidate(active_candidate, next_candidate_index, resume=True)
            next_candidate_index += 1

        if not self.search_stage and not self.direction:
        ...
        for candidate_index in range(next_candidate_index, self.max_candidate_count + 1):
    ...
    def _explore_candidate(self, candidate, candidate_index, resume=False):
        needs_plan = (not resume) or not candidate.priority
        if needs_plan:
            ...
            next_property = (self._pending_properties_for_candidate(candidate) or ["[none]"])[0]
            print(f"  ♻️ 从未通过的下一个性质继续: {next_property}")

        pending_properties = self._pending_properties_for_candidate(candidate)
        seen_properties = set()
...
class AutonomousResearchSystem:
    ...
    def _cache_path_for_pdf(pdf_path):
        cache_dir = str(MARKDOWN_CACHE_DIR)
    ...
    def _build_literature_documents(self, pdf_path, raw_md):
        ...
        extra_patterns = [str(MARKDOWN_CACHE_DIR / "*.md")]
        extra_patterns.extend(
            resolve_glob_from_project(pattern)
            for pattern in LITERATURE_EXTRA_MARKDOWN_GLOBS
    ...
    def _initialize_literature_rag(self, pdf_path, raw_md):
        ...
        try:
            ...
            self.literature_rag = None
            self.literature_rag_warning = str(exc).strip() or "unknown LiteratureRAG initialization error"
            print(
                "⚠️ LiteratureRAG 不可用，已降级为仅使用文献摘要上下文。"
                f" reason={self.literature_rag_warning}",
                flush=True,
            )
    ...
    def _customize_team(self, macro_goal, literature_context):
        ...
        with open(LEGACY_PROMPT_SNAPSHOT_FILE, "w", encoding="utf-8") as handle:
            json.dump(
                {
                    "warning": "Runtime-generated artifact only. Edit app_config.py or agent role defaults instead.",
                    "snapshot_file": str(PROMPT_SNAPSHOT_FILE),
                    "editable_source": snapshot_payload["editable_source"],
                },
                handle,
                ensure_ascii=False,
                indent=2,
    ...
    def _generate_reviewer_directive(self, reviewer, context, draft, stage_label="subtask review"):
        ...
        if "q6" in str(context or "").lower():
            q6_contract = (
                "9) If a Q6 draft claims it can push the bound to 1.98, support rho < 1.98, or establish any other global upper-bound improvement, "
    ...
    def _proposition_closure_reason(self, property_name, proposition_id, draft):
        ...
        if self._verification_needs_closed(verification_needs):
            return ""
        pid = str(proposition_id or "[missing]").strip() or "[missing]"
        return f"{property_name} proposition {pid} still leaves open Verification Needs"

    @staticmethod
    def _tool_report_status_summary(tool_reports):
        summary = []
        for item in tool_reports or []:
            request = item.get("request") or {}
            report = item.get("report") or {}
            spec = request.get("spec") or {}
            summary.append(
                {
                    "request_id": str(request.get("request_id", "")).strip() or "[missing]",
                    "status": str(report.get("status", "")).strip().lower() or "missing",
                    "mode": str(report.get("mode", "") or spec.get("mode", "")).strip().lower() or "unknown",
                }
            )
        return summary

    def _evaluate_q5_tool_certificate(self, proposition, tool_reports):
        requires_tool = self._parse_bool_flag((proposition or {}).get("requires_tool"))
        tool_plan = (proposition or {}).get("tool_plan") or {}
        must_certify = self._parse_bool_flag(tool_plan.get("must_certify")) if requires_tool else False
        summary = self._tool_report_status_summary(tool_reports)
        if not requires_tool:
            return {
                "complete": False,
                "retryable": False,
                "must_certify": False,
                "reason": "Q5 proposition does not declare requires_tool=true",
                "statuses": summary,
            }
        if not summary:
            return {
                "complete": False,
                "retryable": True,
                "must_certify": must_certify,
                "reason": "Q5 proposition requires a numeric certificate, but no tool report was produced",
                "statuses": summary,
            }
        ...
        if any(status == "verified_fail" for status in statuses):
            return {
                ...
                "retryable": False,
                "must_certify": must_certify,
                "reason": "at least one Q5 tool request produced a counterexample",
                "statuses": summary,
            }

        retryable_statuses = {"tool_error", "unavailable", "inconclusive", "analysis_complete", "missing", "unknown"}
        nonpassing = [item for item in summary if item["status"] != "verified_pass"]
        numeric_passes = [item for item in summary if item["status"] == "verified_pass" and item["mode"] == "numeric_1d"]

        if must_certify and nonpassing:
            return {
                "complete": False,
                "retryable": all(item["status"] in retryable_statuses for item in nonpassing),
                "must_certify": True,
                "reason": (
                    "Q5 requires every must-certify tool request to finish with verified_pass; "
                    f"observed statuses={summary}"
                ),
                "statuses": summary,
            }

        if not numeric_passes:
            return {
                "complete": False,
                "retryable": True,
                "must_certify": must_certify,
                "reason": "Q5 requires at least one verified_pass numeric_1d certificate",
                "statuses": summary,
            }
        ...
        if nonpassing:
            return {
                ...
                "retryable": all(item["status"] in retryable_statuses for item in nonpassing),
                "must_certify": must_certify,
                "reason": f"Q5 still has non-passing supplemental tool requests: {summary}",
                "statuses": summary,
            }
    ...
    def _design_candidate(self, goal, literature_context, direction, candidate_index):
        ...
        raw_candidate_id = str(payload.get("candidate_id", "")).strip() or f"candidate_{candidate_index:03d}"
        candidate_id = self.memory.make_unique_candidate_id(
            raw_candidate_id,
            fallback_prefix=f"candidate_{candidate_index:03d}",
        )
        ...
        if candidate_id != raw_candidate_id:
            candidate.artifacts["candidate_id_alias"] = json.dumps(
                {
                    "requested_candidate_id": raw_candidate_id,
            ...
            candidate.append_log(
                "candidate_id_adjusted",
                "candidate_id was adjusted to avoid colliding with existing memory",
                requested_candidate_id=raw_candidate_id,
                assigned_candidate_id=candidate_id,
            )
    ...
    def _request_proposition_tool_requests(
        ...
        retry_block = ""
        if str(retry_feedback or "").strip():
            retry_block = (
    ...
    @staticmethod
    def _tool_request_retry_feedback(reason, requests=None, reports=None):
        parts = [str(reason or "").strip() or "The previous tool-request attempt failed."]
        if requests:
            parts.append(
                "Previous tool requests:\n"
                + json.dumps(list(requests), ensure_ascii=False, indent=2)
            )
        if reports:
            summary = []
            for item in reports:
            ...
            parts.append(
                "Tool execution results from the failed attempt:\n"
    ...
    def _execute_proposition_tool_requests(
        ...
        for item in requests:
            ...
            if status == "verified_fail":
                ...
                return merged.strip(), report_payloads, any_verified_pass
        ...
        if rendered_chunks:
            ...
            draft = clean_draft
        return draft.strip(), report_payloads, any_verified_pass
    ...
    def _apply_proposition_tool_requests(
        ...
        if property_name == "Q5" and self._parse_bool_flag((proposition or {}).get("requires_tool")):
            certificate_result = self._evaluate_q5_tool_certificate(proposition, report_payloads)
            if certificate_result["complete"]:
                return draft, report_payloads, True
        elif certified:
            return draft, report_payloads, True

        retriable_statuses = {"tool_error", "unavailable", "inconclusive", "analysis_complete"}
        observed_statuses = {
            str((item.get("report") or {}).get("status", "")).strip().lower() or "missing"
            for item in report_payloads
        }
        should_retry = bool(report_payloads) and (
            any(status in retriable_statuses for status in observed_statuses)
            or bool(certificate_result and certificate_result.get("retryable"))
        )
        if not should_retry:
            return draft, report_payloads, False
        ...
        retry_feedback = self._tool_request_retry_feedback(
            (
                certificate_result.get("reason")
                if certificate_result and certificate_result.get("reason")
                else "The previous tool execution did not produce a usable certificate because the tool spec failed to execute cleanly."
            ),
            requests=requests,
        ...
        repaired_draft, repaired_reports, repaired_any_verified = self._execute_proposition_tool_requests(
            candidate,
            property_name,
            proposition,
            proposition_meta,
        ...
        if candidate.status == "pruned":
            return repaired_draft, repaired_reports, False
        if property_name == "Q5" and self._parse_bool_flag((proposition or {}).get("requires_tool")):
            repaired_certificate = self._evaluate_q5_tool_certificate(proposition, repaired_reports)
            return repaired_draft, repaired_reports, bool(repaired_certificate.get("complete"))
        return repaired_draft, repaired_reports, repaired_any_verified
    ...
    def _run_property_proposition(
        ...
        for attempt in range(max(1, int(PROPOSITION_REVIEW_MAX_ROUNDS or 1))):
            ...
            if current_round_pass:
                ...
                reason = self._proposition_closure_reason(
                    property_name,
                    proposition.get("id", ""),
                    draft,
                )
    ...
    def _run_candidate_property(self, candidate, property_name, goal, literature_context):
        ...
        for index, (proposition, proposition_draft) in enumerate(proposition_outputs, start=1):
            closure_reason = self._proposition_closure_reason(
                property_name,
                proposition.get("id", ""),
                proposition_draft,
            )
            if closure_reason:
                candidate.mark_property(property_name, "fail", note=closure_reason)
                candidate.mark_pruned(closure_reason)
                candidate.append_log(
                    "prune",
                    "property blocked because a supposedly passed proposition still had open verification needs",
                    proposition_id=proposition.get("id", ""),
                )
                return False
