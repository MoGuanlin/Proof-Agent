...
- Property Snapshot: N1=pass, N2=pass, N3=pass, D4=pass, Q5=fail, Q6=hypothesis
- Terminal Decision: continue_exploring
- Decision Rationale: The previous candidatesfailed N3 because they SUBTRACTED a penalty that decreases with distance, making the potential subadditive. By reversingthe sign and ADDING a rational penalty, the potential becomes strictly superadditive, trivially satisfying N3 globally. Furthermore, therational form $c_2 r^2 / (r + d)$ has a bounded derivative with respect to $d$, ensuringperfect decoupling for D4. This provides a mathematically sound foundation to manipulate the effective $\rho$ and $c$ at theQ5 bottleneck without structural failures.
...
Post-Terminal Decision:
- action: continue_exploring
- rationale: The previous candidatesfailed N3 because they SUBTRACTED a penalty that decreases with distance, making the potential subadditive. By reversingthe sign and ADDING a rational penalty, the potential becomes strictly superadditive, trivially satisfying N3 globally. Furthermore, therational form $c_2 r^2 / (r + d)$ has a bounded derivative with respect to $d$, ensuringperfect decoupling for D4. This provides a mathematically sound foundation to manipulate the effective $\rho$ and $c$ at theQ5 bottleneck without structural failures.
- next_direction: Implement and evaluate Positive_Rational_Penalty_v1 withthe form \Phi(x, y) = \rho |xy| + c_1(r_x + r_y) + c_2 \frac{r_x^2}{r_x + |xy|} + c_2 \frac{r_y^2}{r_y + |xy|}. Focus the proposition design on verifying N3superadditivity and analyzing the N2/Q5 effective parameter shifts.
```

### Cross_Rational_Penalty_v1
- Status: pruned
- Source Direction: Implement and evaluate Positive_Rational_Penalty_v1 withthe form \Phi(x, y) = \rho |xy| + c_1(r_x + r_y) + c_2 \frac{r_x^2}{r_x + |xy|} + c_2 \frac{r_y^2}{r_y + |xy|}. Focus the proposition design on verifying N3superadditivity and analyzing the N2/Q5 effective parameter shifts.
- Derived From: Baseline linear potential + cross-term rational penalty
- Property Snapshot: N1=hypothesis, N2=hypothesis, N3=fail, D4=hypothesis, Q5=hypothesis, Q6=hypothesis
- Terminal Decision: stop
- Decision Rationale: After extensive exploration of various potential functions (projection, harmonic mean, Euclidean norm, rational distance, cross-term penalties), we have hit a fundamental structural bottleneck in the proof framework's local inequalities.

To push the stretch factor $\rho$ below 1.98, we must satisfy the N2 (single-disk removal) condition, which requires the potential $\Phi$ to decrease sufficiently when the distance $d$decreases and the radius $r_x$ shrinks. For small $d$, the baseline potential $\rho d +c(r_x + r_y)$ fails to provide enough negative slack unless $c$ is large. Thus, we need a penalty term $f(r_x, d)$ where $\frac{\partial f}{\partial r_x} > 0$ to boost the effective $c$.

However, any function$f(r_x, d)$ that strictly increases with $r_x$ must be positive. In the N3 (chain split) condition, the segment is split by an intermediate disk of radius $r_i$. The potentialdifference includes $f(r_i, d_1) + f(r_i, d_2)$.If the verifier evaluates N3 for 'grazing intersections' (where $r_i \to \infty$but the chord $|zw| \to 0$), these intermediate terms either diverge to $+\infty$ (if $f$ is linear in $r$) or leave a strictly positive residue (if $f$ is bounded). In either case, the sumexceeds the required upper bound $\Delta \Phi \le -|zw| \approx 0$, strictly violating N3.

Mathematically, no separable penalty function can simultaneously:
1. Provide a positive derivative w.r.t $r_x$ at $d \to 0$ (required for N2).
2. Vanish or remain subadditive for grazing intermediate disks where $r_i \to \infty$ and $d_1+d_2 \approx d$ (required for N3).

In Xia's original proof, this contradictionis avoided because valid Delaunay chain splits only occur when the intersection angle $\theta \ge \theta_0$, whichgeometrically bounds $r_i \le K |zw|$. This prevents $r_i$ from being arbitrarily large when$|zw| \to 0$. However, the current verification environment evaluates N3 as a general structural property without thistopological constraint, causing any $r_x$-dependent penalty to fail.

The most worthwhile next direction is to formally integratethe topological constraint $r_i \le \frac{|zw|}{2 \sin \theta_0}$ into the N3verification protocol. Until the environment restricts the intermediate disk radius based on the chord length, no potential function can mathematically bypass the N2/N3 trade-off to achieve $\rho < 1.98$.

```text
Candidate ID: Cross_Rational_Penalty_v1
Status: pruned
Form: \Phi(x, y) = \rho |xy| + c_1(r_x + r_y) + c_2 \frac{r_xr_y}{r_x + r_y + |xy|}
Derived From: Baseline linear potential + cross-term rational penalty
Intuition: A fundamental limitation of separable penalties f(r_x, d) + f(r_y, d) is that N3 segment subadditivityforces the effective c to be bounded by \rho-1 even for small d, while N2 requires a large effective c(e.g., c > 1.1) for small d. This symmetric cross-term penalty elegantly breaks this barrier. For N3, splitting the segment introduces a small intermediate disk r_i, and the cross term goes to0 as r_i \to 0, providing strictly negative slack and trivially passing the N3 bottleneck without inflating the intermediatedisk penalty. For N2, when d \to 0 and r_y \gg r_x, thederivative with respect to r_x yields a full +c_2 boost to the effective c, exactly where the N2 geometric bottleneck occurs. This allows us to use a smaller c_1 \le \rho-1 for N3 while maintaininga large c_{eff} \approx c_1 + c_2 for N2, making \rho <1.98 achievable within the structural constraints.
Estimated C: 0.0
Risk Notes: The candidate has a fatal structural flaw in N3 for largeintermediate disks. All penalty terms are positive and either grow unboundedly or remain strictly positive as r_i -> infinity,making it impossible to satisfy the subadditivity condition Delta Phi <= -|zw| when the chord |zw| issmall.
Pruned Reason: N3 规划阶段失败: For the N3 subadditivity condition, the potential difference includes 2c_1 r_i + c_2(r_x r_i)/(r_x+r_i+|xz|) + c_2(r_ir_y)/(r_i+r_y+|wy|). As the intermediate disk radius r_i ->infinity, this sum diverges to +infinity if c_1 > 0. Even if c_1 = 0, the sum approaches c_2 r_x + c_2 r_y, so Delta Phi -> -\rho |zw| + c_2(r_x+r_y) - c_2(r_x r_y)/(r_x+r_y+|xy|). For a grazing intersection where the chord |zw| -> 0, the RHS of the required inequality Delta Phi <= -|zw| goes to 0,but the LHS remains strictly positive (since r_x+r_y > (r_x r_y)/(r_x+r_y+|xy|) for r_x, r_y > 0). Thus,N3 is strictly violated for large intermediate disks.
Property Status:
- N1: hypothesis | Promising. The cross term is strictly positive, so \Phi \ge |xy| is trivially maintained.
- N2: hypothesis | Highly Promising. The derivative with respect to r_x adds c_2 \frac{r_y^2 + r_y d}{(r_x + r_y + d)^2} to c_{eff}. For small d and large r_y, this provides a full +c_2 boost, exactly wherethe baseline N2 bottleneck requires a large coefficient. The slight reduction in \rho_{eff} is negligible.
- N3: fail | For the N3 subadditivity condition, the potential difference includes 2c_1 r_i + c_2(r_x r_i)/(r_x+r_i+|xz|) + c_2(r_ir_y)/(r_i+r_y+|wy|). As the intermediate disk radius r_i ->infinity, this sum diverges to +infinity if c_1 > 0. Even if c_1 = 0, the sum approaches c_2 r_x + c_2 r_y, so Delta Phi -> -\rho |zw| + c_2(r_x+r_y) - c_2(r_x r_y)/(r_x+r_y+|xy|). For a grazing intersection where the chord |zw| -> 0, the RHS of the required inequality Delta Phi <= -|zw| goes to 0,but the LHS remains strictly positive (since r_x+r_y > (r_x r_y)/(r_x+r_y+|xy|) for r_x, r_y > 0). Thus,N3 is strictly violated for large intermediate disks.
- D4: hypothesis | Promising. As d \to \infty, the cross term and its derivatives decay to 0, seamlessly recovering the baseline decoupling behavior.
- Q5: hypothesis | Highly Promising. The local continuous condition is satisfied globally because c_{eff} selectively increases exactly when dis small and \alpha is near 0, decoupling the N2 bottleneck from the N3 structural constraint.
- Q6: hypothesis | Promising. The modified exclusion zone only differs by an additive constant bounded by c_2(r_x+r_y), which preserves the structural induction of Lemma 3 for the global stretch factor.
Proposition Status:
[none]
Post-Terminal Decision:
- action: stop
- rationale: After extensive exploration of various potential functions (projection, harmonic mean, Euclidean norm, rational distance, cross-term penalties), we have hit a fundamental structural bottleneck in the proof framework's local inequalities.
